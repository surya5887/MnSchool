...
const lastClosingDiv = `          )}\n        </div>\n      </motion.div>\n    );\n  };\n  \n  export default DefaultersList;`;

const beforeTable = code.substring(0, code.indexOf(tableStart));
const afterTable = lastClosingDiv;

// We need to keep the desktop table
const desktopTableMatch = code.match(/<div className="glass-table-container desktop-only">[\s\S]*?<\/div>/);
if (desktopTableMatch) {
    const desktopTable = desktopTableMatch[0];
    ...
    fs.writeFileSync('src/pages/DefaultersList.tsx', beforeTable + desktopTable + '\n' + newMobileCards + '\n              </>\n' + afterTable, 'utf8');
    console.log("Fixed DefaultersList formatting completely");
} else {
    console.log("Could not extract desktop table");
}
