...
if (code.includes(tableStart) && code.includes(tableEndStr)) {
    const beforeTable = code.substring(0, code.indexOf(tableStart));
    const afterTable = code.substring(code.indexOf(tableEndStr) + tableEndStr.length);
    
    // Extract the exact table code
    const originalTable = code.substring(code.indexOf(tableStart), code.indexOf(tableEndStr) + tableEndStr.length);
