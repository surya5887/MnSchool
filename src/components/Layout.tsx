...
import { LayoutDashboard, Briefcase, Users, LogOut, Bell, GraduationCap, Settings, BookOpen, UserPlus, CalendarCheck, ShieldAlert, FileText, AlertTriangle, Bus, Clock, Library as LibraryIcon, Menu, X, User, Megaphone } from 'lucide-react'; 
  ...
  useEffect(() => {
    if (schoolSettings) {
      if (schoolSettings.logoUrl) {
        ...
        if (link) link.href = schoolSettings.logoUrl;
        
        const appleLink = document.querySelector("link[rel~='apple-touch-icon']") as HTMLLinkElement;
        if (appleLink) appleLink.href = schoolSettings.logoUrl;
      }
      if (schoolSettings.name) {
        document.title = schoolSettings.name;
      }
    }
  }, [schoolSettings]);
        ...
        <nav style={{ display: 'flex', flexDirection: 'column', gap: '8px', flex: 1, overflowY: 'auto' }} onClick={() => setMobileMenuOpen(false)}>
          ...
          {['Admin', 'Principal', 'Manager', 'Super Admin'].includes(role) && (
            <>
              ...
              <NavLink to="/staff" style={navLinkStyle} onClick={() => setMobileMenuOpen(false)}><Briefcase size={20} /> Staff Directory</NavLink>
