...
const ReportCardPrintView: React.FC<ReportCardProps> = ({ students, marks, term, className, section, maxMarks, onClose }) => {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      window.print();
      onClose(); // Automatically close the view after printing dialog closes
    }, 500);
  }, [onClose]);
  ...
  return (
    <div className="report-card-container">
      {students.map((student, idx) => {
        ...
        return (
          <div key={student.id} className="report-card-page" style={{ pageBreakAfter: idx === students.length - 1 ? 'auto' : 'always', padding: '40px', background: 'white', color: 'black', fontFamily: 'Arial, sans-serif' }}>
            {/* School Header */}
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: '30px', border: '1px solid #000' }} className="print-marks-table">
              ...
              {studentMarks.length > 0 && (
                <tfoot>
                  <tr style={{ backgroundColor: '#f3f4f6', WebkitPrintColorAdjust: 'exact' }}>
            ...
            {/* Result Summary */}
            {studentMarks.length > 0 && (
              <div style={{ display: 'flex', justifyContent: 'space-between', padding: '20px', border: '2px solid #1e3a8a', borderRadius: '8px', WebkitPrintColorAdjust: 'exact', backgroundColor: '#eff6ff' }}>
