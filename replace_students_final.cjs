...
// 1. Update the grid template CSS to make cards smaller (250px instead of 280px)
content = content.replace(/minmax\(280px, 1fr\)/g, 'minmax(250px, 1fr)');
// Reduce grid gap
content = content.replace(/gap: '20px'/g, "gap: '16px'");
// Reduce grid padding
content = content.replace(/padding: '24px'/g, "padding: '20px'");

// 2. Locate the motion.div representing the card
const startIndex = content.indexOf(') : filteredStudents.map((student, index) => (\n              <motion.div');
...
if (startIndex !== -1 && endIndex !== -1) {
  ...
  const newCardLayout = `) : filteredStudents.map((student, index) => (
              <motion.div 
                key={student.id}
  ...
  const newContent = content.substring(0, startIndex) + newCardLayout + content.substring(endIndex);
  fs.writeFileSync(filePath, newContent, 'utf8');
  console.log("Card layout replaced perfectly.");
} else {
