import re

file_path = 'src/components/DocumentBuilder.tsx'

with open(file_path, 'r', encoding='utf-8') as f:
    content = f.read()

# Let's completely isolate the contentEditable from framer-motion by putting it in a wrapper div with stopPropagation on every single capture event.

pattern = r"\{el\.type === 'text' \? \([\s\S]*?\) : \("

new_block = """{el.type === 'text' ? (
                <div 
                    onPointerDownCapture={e => e.stopPropagation()}
                    onPointerUpCapture={e => e.stopPropagation()}
                    onMouseDownCapture={e => e.stopPropagation()}
                    onMouseUpCapture={e => e.stopPropagation()}
                    onClickCapture={e => { e.stopPropagation(); setSelectedId(el.id); }}
                    onKeyDownCapture={e => e.stopPropagation()}
                    onKeyUpCapture={e => e.stopPropagation()}
                    style={{ width: '100%', height: '100%', cursor: 'text' }}
                >
                    <div 
                        id={`editor-${el.id}`}
                        contentEditable={!printing}
                        suppressContentEditableWarning
                        style={{ 
                            outline: 'none', 
                            userSelect: 'text',
                            WebkitUserSelect: 'text',
                            cursor: printing ? 'default' : 'text', 
                            width: '100%',
                            height: '100%',
                            minWidth: '100px', 
                            fontSize: '18px', 
                            fontFamily: 'Arial, sans-serif'
                        }}
                        dangerouslySetInnerHTML={{ __html: el.content || 'Click to edit text' }}
                        onBlur={(e) => {
                            const newElements = elements.map(e_inner => e_inner.id === el.id ? { ...e_inner, content: e.currentTarget.innerHTML } : e_inner);
                            setElements(newElements);
                        }}
                    />
                </div>
            ) : ("""

content = re.sub(pattern, new_block, content)

with open(file_path, 'w', encoding='utf-8') as f:
    f.write(content)

print("Isolated contentEditable completely.")
