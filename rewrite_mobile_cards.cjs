...
const startTag = '<div className="mobile-only">';
const endTag = '</div>\n            </div>\n              </>\n          )}\n        </div>';

if (code.includes(startTag)) {
    const startIndex = code.indexOf(startTag);
    // Find where the mobile-only block ends. 
    // It ends right before `</>` which is before `)}`
    
    const fragmentEnd = code.indexOf('</>', startIndex);
    
    if (fragmentEnd !== -1) {
        const before = code.substring(0, startIndex);
        const after = code.substring(fragmentEnd);
        ...
        fs.writeFileSync('src/pages/DefaultersList.tsx', before + newMobileCards + '\n              ' + after, 'utf8');
        console.log("Rewrote mobile cards successfully");
    } else {
        console.log("Could not find fragment end");
    }
} else {
    console.log("Could not find mobile-only start tag");
}
